    ...
    class << self
        ...
        def proc_main(driver, syori_kbn)
            ...
            begin
                ...
                $logger.info("#{SESSIONS::syori_cnt.to_s.rjust(2)} クラウド連携メニューが選択されました。")
                ...
                2.times do |cnt|
                    ...
                    # 処理１～２ 《メモ》 処理１ 00秒   処理２ 14秒
                    driver.find_element(id: "#{id_tag_submit}").click
                    sleep(minutes)
                    
                    SESSIONS::syori_cnt += 1
                    $logger.info("#{SESSIONS::syori_cnt.to_s.rjust(2)} #{msg}を実行しました。")
                    ...
                    # 実行 《メモ》実行 処理１の時 1分28秒 処理２の時 0分00秒
                    begin
                        SESSIONS::syori_cnt += 1
                    ...
                    SESSIONS::syori_cnt += 1
                    $logger.info("#{SESSIONS::syori_cnt.to_s.rjust(2)} #{msg}をクリックしました。")
                end

                return nil
            rescue => ex
                return "method - " + __method__.to_s + " : " + ex.message + ": 「Sofinet Cloudの連携」でエラーが発生しました。"
            end
        end
    end
end
    ...
    class << self
        ...
        def proc_main(driver)
            ...
            begin
                ...
                SESSIONS::syori_cnt += 1
                $logger.info("#{SESSIONS::syori_cnt.to_s.rjust(2)} 入金仕入メニューを選択しました。")

                # データ件数が多くなって処理時間が長くなった場合は、sleep時間を長くする必要がある
                3.times do |cnt|
                ...
                return "method - " + __method__.to_s + " : " + ex.message + ": 「楽楽のCSVインポート」でエラーが発生しました。"
            end
        end
        ...
        def proc_down(driver)
            begin
                ...
                SESSIONS::syori_cnt += 1
                $logger.info("#{SESSIONS::syori_cnt.to_s.rjust(2)} 入金一覧Excelをクリックしました。")
                ...
                SESSIONS::syori_cnt += 1
                $logger.info("#{SESSIONS::syori_cnt.to_s.rjust(2)} 仕入一覧CSV出力をクリックしました。")
                ...
                return "method - " + __method__.to_s + " : " + ex.message + ": 「入金一覧Excel／仕入一覧CSV出力」でエラーが発生しました。"
            end
        end
    end
end
    ...
    class << self
        ...
        def proc_main(driver)
            begin
                ...
                SESSIONS::syori_cnt += 1
                $logger.info("#{SESSIONS::syori_cnt.to_s.rjust(2)} 請求月計算メニューを選択しました。")
                ...
                SESSIONS::syori_cnt += 1
                $logger.info("#{SESSIONS::syori_cnt.to_s.rjust(2)} CSV出力をクリックしました。")
                ...
                return "method - " + __method__.to_s + " : " + ex.message + ": 「請求月計算」でエラーが発生しました。"
            end
        end
    end
end
